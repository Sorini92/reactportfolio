...
const Skills = ({theme}) => {
    ...
    const skills = [
        ...
        {darkimg: "resources/icons/skills/sass.svg", lightimg: "resources/icons/skills/sass.svg", title: "Sass", description: "Sass это расширение CSS, которое придаёт мощи и элегантности этому простому языку. Sass даст вам возможность использовать переменные, вложенные правила, миксины, инлайновые импорты и многое другое."},
        ...
        return (
            <div key={i} className={`${prefix}__item uk-animation-scale-up uk-transform-origin-top-center`}>
                <div className={`${prefix}__logo`}>
                    {theme === "light" ? <img src={item.lightimg} alt={item.title}/> : <img src={item.darkimg} alt={item.title}/>}
                </div>
